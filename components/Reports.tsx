...
export const Reports: React.FC = () => {
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.setAttribute('download', 'sua_mao_de_obra_prestadores.csv');
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-lg hover:bg-slate-800 transition-colors shadow-lg active:scale-95 border-none cursor-pointer"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <p className="text-sm font-medium text-slate-500 mb-1">Total de Profissionais</p>
          <h4 className="text-2xl font-bold text-indigo-600">{providers.length}</h4>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <p className="text-sm font-medium text-slate-500 mb-1">Bloqueados</p>
          <h4 className="text-2xl font-bold text-slate-400">0</h4>
        </div>
      </div>
