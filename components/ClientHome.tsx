...
export const ClientHome: React.FC = () => {
  ...
  const getProviderServices = (ids: string[]) => {
    return ids.map(id => allServices.find(s => s.id === id)?.name).filter(Boolean).slice(0, 2);
  };
    ...
    <div className="space-y-16 animate-in fade-in duration-700 pb-12">
      <section className="text-center space-y-6 pt-6">
        ...
        <p className="text-lg text-slate-500 max-w-2xl mx-auto px-4 leading-relaxed font-medium">
          Encontre os melhores profissionais para sua casa ou empresa. Negocie direto no WhatsApp, sem taxas extras.
        </p>
      </section>
              ...
              <div key={provider.id} className="bg-white p-5 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all flex items-center justify-between group">
                <div className="flex items-center gap-4 overflow-hidden">
                  <div className="w-16 h-16 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-400 font-black overflow-hidden border border-indigo-100 shrink-0">
                    {provider.profileImage ? <img src={provider.profileImage} className="w-full h-full object-cover" /> : <User size={28} />}
                  </div>
                ...
                <button onClick={() => openWhatsApp(provider.phone, provider.name)} className="bg-emerald-500 text-white p-4 rounded-2xl shadow-lg hover:scale-110 transition-all border-none cursor-pointer flex flex-col items-center gap-0.5">
                  <MessageCircle size={22} /><span className="text-[8px] font-black uppercase">Zap</span>
                </button>
              </div>
            ))}
          </div>
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {services.map((service) => (
            <button key={service.id} onClick={handleNavigate(`/busca/${service.id}`)} className="group bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all flex flex-col items-center text-center gap-4 active:scale-95 cursor-pointer">
              ...
              <span className="font-bold text-slate-800 text-sm group-hover:text-indigo-600">{service.name}</span>
            </button>
          ))}
          <button onClick={handleNavigate('/cadastro')} className="group bg-slate-900 p-6 rounded-[2rem] border border-slate-800 shadow-xl hover:bg-slate-800 transition-all flex flex-col items-center justify-center text-center gap-3 active:scale-95 cursor-pointer">
            <div className="bg-slate-800 p-3 rounded-xl"><UserPlus className="w-6 h-6 text-indigo-400" /></div>
            <span className="text-white font-bold text-sm">Trabalhe Conosco</span>
            <span className="text-indigo-400 text-[10px] flex items-center gap-1 font-black uppercase tracking-widest">Começar <ArrowRight size={14} /></span>
          </button>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-8 pt-8 border-t border-slate-200">
        <div className="flex gap-4 items-start">
          <div className="shrink-0 w-12 h-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center"><Zap size={24} /></div>
          <div><h4 className="font-bold text-slate-900">Agilidade</h4><p className="text-xs text-slate-500 font-medium">Conectamos você ao profissional em instantes.</p></div>
        </div>
        <div className="flex gap-4 items-start">
          <div className="shrink-0 w-12 h-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center"><Search size={24} /></div>
          <div><h4 className="font-bold text-slate-900">Transparência</h4><p className="text-xs text-slate-500 font-medium">Negocie direto, sem taxas extras.</p></div>
        </div>
        <div className="flex gap-4 items-start">
          <div className="shrink-0 w-12 h-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center"><CheckCircle2 size={24} /></div>
          <div><h4 className="font-bold text-slate-900">Qualidade</h4><p className="text-xs text-slate-500 font-medium">Apenas prestadores ativos na região.</p></div>
        </div>
      </section>
    </div>
  );
